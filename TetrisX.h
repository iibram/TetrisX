 ...
 */
enum class Movement { ROTATION, DOWN, LEFT, RIGHT };

/**
 * @brief An enum class for the states of the completion routine's "Finite-State Machine" (FSM)
 */
enum class State { IDLE, BOOM_1, BOOM_2, DROP };


/**
 * @brief Represents a cell of the active Tetris block (currBlock)
 */
struct B_Cell
{
	uint16_t idx = 0;									// index on the 1D board array
	uint8_t  lvl = 0;									// distance level to the center of the block
...
struct Configs
{
	...
	constexpr static std::string_view VISUALS[14] =
	{
		...
		"\U00002503", "\U00002517", "\U00002501\U00002501", "\U0000251B",									// boarders [10..13] : ┃┗ ━━ ┛
	};
	...
	*/
	...
	constexpr static std::array<int, 19> SPEEDS = { 50, 45, 41, 37, 33, 30, 27, 24, 21, 19, 17, 15, 13, 11, 9, 8, 7, 6, 5 };
};
...
private:
	...
	//FlatBlock nextBlock;
	std::vector<Cell> board;					// the entire game board (including borders) as a flat 1D array (Cell = {idx, id, solid})
	...
	MoveInfo moveInfo;							// info from movement checks
	...
	uint8_t currID;								// the ID of the current block
	uint8_t nextID;								// the ID of the next block
	uint8_t nIDX;								// next index for the 7-bag system

	State complFSM = State::IDLE;				// control note: completion routine for FSM phases (IDLE, BOOM_1, BOOM_2, DROP)
	uint8_t bottomRow;							// the lowest row from which to check upwards for completed rows
